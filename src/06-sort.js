...
{
  const {runtimeInSeconds} = songs
  const ascendingRuntime = songs.sort( ( current, next ) =>
  {
...
{
   const {artist} = songs
  const descendingArtists = songs.sort( ( current, next ) =>
  {
...
{
   const {title} = songs
  const ascendingSongTitle = songs.sort( ( current, next ) =>
  {
