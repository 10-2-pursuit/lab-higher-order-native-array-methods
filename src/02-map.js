...
function getSongTitles(songs) {
  let allSongTitles = songs.map((song) => {
    return song.title
  })
...
function getSongDetails(songs) {
  let allSongTitlesAndArtists = songs.map((song) => {
...
function getTitleAndArtist(songs) {
  let songTitleAndArtistObject = songs.map((song) => {
    let newObject = {}
    newObject[song.title] = song.artist
    return newObject
  })
  return songTitleAndArtistObject
}
