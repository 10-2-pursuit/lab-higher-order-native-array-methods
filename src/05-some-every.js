...
{
  const overTwoMinutes = songs.every( songs => songs.runtimeInSeconds > 120 )
...
{
  const overFourMinutes = songs.some( songs => songs.runtimeInSeconds > 240 )
...
{
  const peanutSong = songs.some( songs => songs.artist.includes( "Peanut" ) )
  return peanutSong
}
