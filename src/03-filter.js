...
{
  const saibSongs = songs.filter( songs => songs.artist === "Saib" )
  return saibSongs
}
...
{
  const overThreeMinutes = songs.filter( songs => songs.runtimeInSeconds > 180 )
...
{
  const sameAlbum = songs.filter( songs => songs.album === songs.title );
