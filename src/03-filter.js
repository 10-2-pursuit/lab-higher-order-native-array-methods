...
function getSongsBySaib(songs) {
  let filterByArtist = songs.filter(song => song.artist.includes("Saib"));
...
function getSongsOverThreeMinutes(songs) {
  let filterByRuntime = songs.filter(song => song.runtimeInSeconds > 180);
  return filterByRuntime;
}
...
function getTitleTracks(songs) {
  let filterBySameName = songs.filter(song => song.title === song.album);
  return filterBySameName;
}
